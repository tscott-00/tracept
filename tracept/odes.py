...
def integrator_step(i, args, fstep, upd_dyn):
...
def make_integrator(z_meta, fstep, upd_dyn=None):
    ...
    _integrator_step = jax.jit(partial(integrator_step, fstep=fstep, upd_dyn=upd_dyn))
    
    def _integrator(z0, dt, T, _integrator_step=_integrator_step, dmap_z_I=dmap_z_I, dmap_dz_I=dmap_dz_I):
        if type(z0) is TWrapper:
        ...
        return t, TWrapper(z_dyn_stack, z_tree, is_root=True)
